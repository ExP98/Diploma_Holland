...
```{python}
def evaluate_losses(losses: dict, model_cls, num_epochs=100,
...
```{python}
mlp_results = evaluate_losses(losses, MLPRanker, num_epochs=100)

for item in mlp_results.items():
    print(item)
```

```{python}
nl_mlp_results = evaluate_losses(losses, NonlinearMLPRanker, num_epochs=100)

for item in nl_mlp_results.items():
    print(item)
```

```{python}
dcn_results = evaluate_losses(losses, DCNUserItemRanker, num_epochs=100)

for item in dcn_results.items():
    print(item)
```

```{python}
lwt_results = evaluate_losses(losses, ListwiseTransformer, num_epochs=100)

for item in lwt_results.items():
    print(item)
```
