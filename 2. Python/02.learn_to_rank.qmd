...
```{python}
listnet_model = ListNet()
listnet_model = train_model(listnet_model, listnet_loss, X_train, y_train, num_epochs=100)

LTR_LN_preds = predict_scores(listnet_model, X_test)
print(f"C-index: {round(df_metric(LTR_LN_preds, y_test, calc_C_index), 3)}")
print(f"NDCG@k: {ndcg_at_k(LTR_LN_preds, y_test, k=3)}")
```
...
```{python}
ApproxNDCG_model = DeepRanker() # input_dim=X_train.shape[1]
ApproxNDCG_model = train_model(ApproxNDCG_model, approx_ndcg_loss, X_train, y_train, num_epochs=100)

AN_preds = predict_scores(ApproxNDCG_model, X_test)
print(f"C-index: {round(df_metric(AN_preds, y_test, calc_C_index), 3)}")
print(f"NDCG@k: {ndcg_at_k(AN_preds, y_test, k=3)}")
```
...
```{python}
ll_model = DeepRanker()
ll_model = train_model(ll_model, lambda_rank_loss, X_train, y_train, num_epochs=100)

ll_preds = predict_scores(ll_model, X_test)
print(f"C-index: {round(df_metric(ll_preds, y_test, calc_C_index), 3)}")
print(f"NDCG@k: {ndcg_at_k(ll_preds, y_test, k=3)}")
```

Архитектура: простая последовательность полносвязных слоёв (MLP) с BatchNorm, Dropout, ReLU.

Обработка: каждый документ (item) обрабатывается независимо — модель выдаёт скоринг si=f(xi) без учёта остальных элементов списка.

Особенности:
    Минимальная способность моделировать зависимость между элементами.
    Легковесен и быстр в вычислении, легко масштабируется на большие батчи.
    Подходит в качестве baseline, когда списки короткие и зависимости между элементами несущественны.
...
```{python}
LWT = ListwiseTransformerUI()
LWT = train_model(LWT, lambda_rank_loss, X_train, y_train)

LWT_preds = predict_scores(LWT, X_test)
print(f"C-index: {round(df_metric(LWT_preds, y_test, calc_C_index), 3)}")
print(f"NDCG@k: {ndcg_at_k(LWT_preds, y_test, k=3)}")
```

Архитектура: поверх входных векторов каждого item сначала проекция в пространство размерности d, затем несколько слоёв TransformerEncoder (self‑attention), и в конце линейный выход.

Обработка: всё n-элементное «предложение» подаётся как единый тензор (batch,n,d). Модель через механизм внимания явно учитывает, как каждый item соотносится со всеми остальными.

Плюсы:
    Позволяет захватывать «конкуренцию» внутри списка, перенастраивая скоринг одного элемента в зависимости от соседей.
    Гибко масштабируется на разные длины списков (через positional‑encoding, если нужен порядок).

Минусы:
    O(n2) в памяти и вычислениях из‑за попарных attention‑весов.
    Чувствителен к гиперпараметрам трансформера (число голов, слоёв).
...
```{python}
dcn_model = DCNUserItemRanker()
dcn_model = train_model(dcn_model, lambda_rank_loss, X_train, y_train)

dcn_preds = predict_scores(dcn_model, X_test)
print(f"C-index: {round(df_metric(dcn_preds, y_test, calc_C_index), 3)}")
print(f"NDCG@k: {ndcg_at_k(dcn_preds, y_test, k=3)}")
```

Архитектура: для каждого item комбинирует два «потока»
    Cross‑Network: рекурсивные «кросс‑слои», которые моделируют мультипликативные и полиномиальные пересечения признаков, сохраняя при этом тот же размер.
    Deep‑Network: классический MLP. Затем конкатенирует выходы обоих потоков и выдаёт скоринг.

Обработка: по‑прежнему каждый item обрабатывается самостоятельно, но внутри него усиливаются сложные feature‑interactions.

Плюсы:
    Очень эффективно моделирует высокоуровневые пересечения признаков без экспоненциального роста числа параметров.
    Хорошо работает, когда у вас много категориальных/биннинговых фич и важны их комбинации.

Минусы:
    По сравнению с классическим MLP чуть более тяжеловесен.
    Не учитывает взаимовлияние разных items внутри одной группы.
