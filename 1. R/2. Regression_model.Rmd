...
for (hol_i in 1:6) {
  ...
  for (i in 1:length(folds)) {
    ...
    cat_model <- catboost.train(pool_train, NULL, params = list(iterations = 100, loss_function = 'RMSE', depth = 6, logging_level = "Silent"))
    base_preds[fold_idx, "catboost"] <- catboost.predict(cat_model, pool_val)
    
    # Random Forest
    rf_model <- randomForest(X_fold_train, y_fold_train, ntree = 100)
  ...
  final_rf <- randomForest(X_train, y_train, ntree = 100)
  ...
  cat_final <- catboost.train(pool_full, NULL, params = list(
    iterations = 100, loss_function = 'RMSE', depth = 6, logging_level = "Silent"))
