...
```{r}
cooccurrence <- function(y) {
  co_mat <- t(y) %*% y
  return(co_mat)
}

co_mat <- cooccurrence(Y_b_train)
```
...
predict_by_linkage_metrics <- function(cooccur_mat, first_inds_vec = NULL, avg_prob_b = TRUE, n_repeat = 10^2) {
  rnd_max_score <- sapply(1:nrow(Y_test),
                          \(i) calc_C_index_threes(
                            predict_three_rnd_max(cooccur_mat, first_inds_vec[i]),
                            get_max_indx(Y_test[i, ])
                          )) %>% mean()
  
  # rnd_prob_score <- sapply(1:nrow(Y_test),
  #                          \(i) calc_C_index_threes(
  #                            predict_three_rnd_prob(cooccur_mat, first_inds_vec[i]),
  #                            get_max_indx(Y_test[i, ])
  #                          )) %>% mean()
...
```{r}
dtrain <- wide_data[split_idx] %>% prepare_ranking_data() %>% prepare_model_data()
test_df <- wide_data[!split_idx] %>% prepare_ranking_data()
dtest <- test_df %>% prepare_model_data()
...
data <- wide_data %>%
  copy() %>% 
  mutate(acrossa(HL_1:HL_6, ~rank(-., ties.method = "random"), .names = "rank_{.col}")) %>% 
  as_tibble()

# Разделение данных
set.seed(42)

model <- xgboost(data = X_train, label = y_train, nrounds = 500, verbose = 0,
                 objective = "rank:pairwise", eval_metric = "ndcg@3")

pred_scores <- predict(model, X_test)

pred_ranks <- apply(matrix(pred_scores, ncol = 6), 1, function(x) rank(-x, ties.method = "random"))
```
