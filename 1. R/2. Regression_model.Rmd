...
df_rmse <- function(pred_df, Y_test_) {
  return(sapply(1:nrow(Y_test_), \(i) my_rmse(pred_df[i, ], Y_test_[i, ])) %>% mean())
}
...
MO -- multioutput
```{r}
perform_stack_MO_regression <- function(model_class, print_msg = "", 
  ...
  if (print_metric) print(show_custom_metrics(ind_pred, paste0(print_msg, " stack"), Y_test_ = Y_test_))
  return(invisible(ind_pred))
}


perform_chain_MO_regression <- function(model_class, print_msg = "", 
                                        X_train_, Y_train_, X_test_, Y_test_,
                                        print_metric = TRUE) {
  models <- vector("list", 6)
...
# perform_stack_MO_regression(my_XGBoost_model, "XGBoost", X_train, Y_train, X_test, Y_test)
# perform_chain_MO_regression(my_XGBoost_model, "XGBoost", X_train, Y_train, X_test, Y_test)
```
...
x <- models_df %>% 
  copy() %>% 
  .[, stack := map(model, \(mdl) perform_stack_MO_regression(mdl, name, X_train, Y_train, X_test, Y_test))] %>% 
  .[, chain := map(model, \(mdl) perform_chain_MO_regression(mdl, name, X_train, Y_train, X_test, Y_test))] %>% 
  .[, (c("stack_rmse", "chain_rmse")) := map(.SD, \(col) map(col, \(pred) df_metric(pred, Y_test, my_rmse))),
    .SDcols = c("stack", "chain")] %>% 
  .[, (c("stack_cos", "chain_cos")) := map(.SD, \(col) map(col, \(pred) df_metric(pred, Y_test, cosine_dist))),
    .SDcols = c("stack", "chain")]

x[1, rmse]
# sapply(1:nrow(Y_test_), \(i) my_rmse(my_pred[i, ], Y_test_[i, ])) %>% mean()
```
