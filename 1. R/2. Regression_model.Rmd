...
.[catb_ind_pred, catb_chain_pred] <- map(
  list(perform_indep_multioutput_regression, perform_chain_multioutput_regression), exec,
  my_Catboost_model, "CatBoost", X_train, Y_train, X_test, Y_test
)
```
...
```{r}
models_pred <- list(const_pred, xgb_ind_pred, xgb_chain_pred, lgbm_ind_pred, lgbm_chain_pred, lgbm_cv_ind_pred, lgbm_cv_chain_pred, rf_ind_pred, rf_chain_pred, lm_ind_pred, catb_ind_pred, catb_chain_pred)
...
c("BF", "EY", "CT", "LN", "SC")
```{r}
df <- prepare_psytest_dataset(wide_data, wide_data2, "CT", use_PCA_b = F)
...
```{r}
models_pred <- list(x[[1]][[1]], x[[2]][[1]], x[[3]][[2]], x[[4]][[1]], x[[5]][[1]], x[[6]][[1]])
...
hamming_loss(Y_b_test, Y_pred)
hamming_loss(Y_b_test, Y_rand)
...
```{r}
# Расчет точности
mean(rowSums(Y_pred * Y_b_test) >= 1)  # Хотя бы одна верная
mean(rowSums(Y_pred * Y_b_test) >= 2)  # Хотя бы 2 верные
mean(rowSums(Y_pred * Y_b_test) >= 3)  # Все 3 верные
...
subset_accuracy(Y_b_test, Y_pred)
```

```{r}
# Расчет точности
mean(rowSums(Y_rand * Y_b_test) >= 1)  # Хотя бы одна верная
mean(rowSums(Y_rand * Y_b_test) >= 2)  # Хотя бы 2 верные
mean(rowSums(Y_rand * Y_b_test) >= 3)  # Все 3 верные
...
```{r}
calc_multiclass_metrics(Y_b_test, Y_pred)
```

```{r}
calc_multiclass_metrics(Y_b_test, Y_rand)
```
...
jaccard_score(Y_b_test, Y_pred)
jaccard_score(Y_b_test, Y_rand)
```
