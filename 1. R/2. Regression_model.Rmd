...
my_XGBoost_model <- R6Class(
  ...
  public = list(
    fit = function(X_train_, y_train_) {
      self$model <- xgboost(data = X_train_, label = y_train_, nrounds = 15, 
...
```{r}
indep_xgb <- vector("list", 6)
for (feature_num in 1:6) {
  indep_xgb[[feature_num]] <- my_XGBoost_model$new(X_train, Y_train[, feature_num], X_test)
}

xgb_ind_pred <- sapply(indep_xgb, \(x) x$pred_test) %>% prediction_correction()
show_custom_metrics(xgb_ind_pred, "XGBoost независимое")
```
...
for (col_i in 1:6) {
  if (col_i != 1) {
    local_train_set <- cbind(local_train_set, col_i = models[[col_i-1]]$pred_train)
    local_test_set <- cbind(local_test_set, col_i = models[[col_i-1]]$pred_test)
  ...
  models[[col_i]] <- my_XGBoost_model$new(local_train_set, Y_train[, col_i], local_test_set, Y_test[, col_i])
  print(models[[col_i]]$rmse_test)
}

xgb_chain_pred <- sapply(models, \(x) x$pred_test) %>% prediction_correction()
show_custom_metrics(xgb_chain_pred, "XGBoost Chained")
```
...
lgb_ind_pred <- indep_lgbm_pred %>% prediction_correction()
show_custom_metrics(lgb_ind_pred, "LGBM независимое")
```
...
my_rmse(Y_test[, col_i], pred_test_i)
...
for (col_i in 2:6) {
  ...
  all_test_preds[[col_i]] <- pred_test_i
  print(my_rmse(Y_test[, col_i], pred_test_i))
}

lgb_chain_pred <- all_test_preds %>% prediction_correction()
show_custom_metrics(lgb_chain_pred, "LGBM Chained")
```
...
for (feature_num in 1:6) {
  ...
  rf_pred[[feature_num]] <- predict(models, data.frame(X_test, Y_test))
}

rf_ind_pred <- rf_pred %>% prediction_correction()
show_custom_metrics(rf_ind_pred, "Random Forest независимое")
```
...
rf_chain_pred <- all_test_preds %>% prediction_correction()
show_custom_metrics(rf_chain_pred, "Random Forest Chained")
```
...
lm_ind_pred <- predict(lm_model, newdata = data.frame(X_test, Y_test)) %>% prediction_correction()
...
for (feature_num in 1:6) {
  train_pool <- catboost.load_pool(data = X_train, label = Y_train[, feature_num])
  test_pool  <- catboost.load_pool(data = X_test,  label = Y_test[, feature_num])
...
catb_ind_pred <- indep_catb_pred %>% prediction_correction()
show_custom_metrics(catb_ind_pred, "Catboost независимое")
```
...
catb_chain_pred <- all_test_preds %>% prediction_correction()
show_custom_metrics(catb_chain_pred, "Catboost Chained")
```
...
for (feature_num in 1:6) {
  ...
  rf_pred[[feature_num]] <- predict(models[[feature_num]], data.frame(X_test_pca_reduced, Y_test))
}
...
for (feature_num in 1:6) {
  xgb_model <- xgboost(data = X_train_pca_reduced, label = Y_train[, feature_num],
                       nrounds = 15, objective = "reg:squarederror", eval_metric = "rmse",
                       verbose = 0)
  
  indep_pred[[feature_num]] <- predict(xgb_model, X_test_pca_reduced)
}
...
indep_catb_pred <- vector("list", 6)
for (feature_num in 1:6) {
  train_pool <- catboost.load_pool(data = X_train_pca_reduced, label = Y_train[, feature_num])
  test_pool  <- catboost.load_pool(data = X_test_pca_reduced,  label = Y_test[, feature_num])
