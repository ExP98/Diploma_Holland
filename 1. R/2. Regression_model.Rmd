...
models_df <- data.table(
  model = c(my_lm_model, NA, NA, my_XGBoost_model, my_LightGBM_model, my_Catboost_model,
            my_RandomForest_model, my_knn_model, my_SVR_model),
  name = c("lm", "Lasso L1", "Ridge L2", "XGBoost", "LightGBM", "CatBoost", 
           "Random Forest", "kNN", "SVR"),
  need_MO = c(T, F, F, rep(T, 6))
)
...
MO_res <- models_df %>% 
  ...
  .[, rmse := map_dbl(pred, \(x) df_metric(x, Y_test, my_rmse))] %>% 
  .[, cos := map_dbl(pred, \(x) df_metric(x, Y_test, cosine_dist))] %>% 
  .[map_lgl(pred, \(item) !is.null(item)), C_index := map_dbl(pred, \(x) df_metric(x, Y_test, calc_C_index))]
```
...
```{r}
get_L1_L2_glmnet_preds(alpha = 0) # Ridge
get_L1_L2_glmnet_preds(alpha = 1) # Lasso
```
...
## 3.8 SVR
```{r}
perform_stack_MO_regression(my_SVR_model, X_train, Y_train, X_test, Y_test, T)
perform_chain_MO_regression(my_SVR_model, X_train, Y_train, X_test, Y_test, T)
```
