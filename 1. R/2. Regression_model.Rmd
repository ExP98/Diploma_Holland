...
my_kNN <- function(X_test_, X_train_, Y_train_, k_nn = 43,
...
k = 1:
```{r}
my_kNN(X_test, X_train, Y_train, dist_func = cosine_dist, k_nn = 1) %>% 
  show_custom_metrics("kNN. Cosine")

my_kNN(X_test, X_train, Y_train, dist_func = my_rmse, k_nn = 1) %>% 
  show_custom_metrics("kNN RMSE")
```

Лучше использовать median (вместо mean), потому что:
```{r}
knn_avg_func_test <- function(avg_func, 
                              X_test_ = X_test, X_train_ = X_train,
                              Y_train_ = Y_train, Y_test_ = Y_test, 
                              k_to_explore = 2:100) {
  res_kNN <- sapply(k_to_explore, function(k_nn) {
    pred.kNN <- my_kNN(X_test_, X_train_, Y_train_, k_nn = k_nn, avg_func = avg_func) 
    list(
