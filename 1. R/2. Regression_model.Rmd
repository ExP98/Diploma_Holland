...
## 9.3 Nearest neigbour

### 1 ближайший сосед

Показывать результат ближайшего объекта, затем и кластера
```{r}
find_nearest_neighbour <- function(input_vec, X_train_, Y_train_, dist_func = my_rmse) {
...
oneNN <- function(X_test_, X_train_, Y_train_, dist_func = my_rmse, 
                  need_dim_red = TRUE, k_dim = 38) {
...
```{r}
oneNN(X_test, X_train, Y_train, cosine_dist, need_dim_red = FALSE) %>% 
  show_custom_metrics("OneNN. Cosine")

oneNN(X_test, X_train, Y_train, my_rmse, need_dim_red = FALSE) %>% 
...
res_1NN <- sapply(k_to_explore, function(k_dim) {
  pred.1NN <- oneNN(X_test, X_train, Y_train, my_rmse, need_dim_red = T, k_dim = k_dim) 
  list(
...
```{r}
find_K_nearest_neighbour <- function(input_vec, X_train_, Y_train_, k_nn = 5,
...
my_kNN <- function(X_test_, X_train_, Y_train_, dist_func = my_rmse, k_nn = 1,
                   avg_func = median, need_dim_red = TRUE, k_dim = 38) {
  if (need_dim_red == TRUE) {
    pca_model <- prcomp(X_train_, center = TRUE, scale. = TRUE)
    X_train_ <- pca_scaler(X_train_, pca_model, k = k_dim)
    X_test_  <- pca_scaler(X_test_, pca_model, k = k_dim)
...
```{r}
my_kNN(X_test, X_train, Y_train, cosine_dist) %>% show_custom_metrics("kNN. Cosine")
my_kNN(X_test, X_train, Y_train, my_rmse) %>% show_custom_metrics("kNN RMSE")
```
...
res_kNN <- sapply(k_to_explore, function(k_nn) {
  pred.kNN <- my_kNN(X_test, X_train, Y_train, my_rmse, k_nn = k_nn, avg_func = median) 
  list(
    rmse = df_metric(pred.kNN, Y_test, my_rmse),
    cind = df_metric(pred.kNN, Y_test, calc_C_index)
  )
}) %>% 
  t() %>% as.data.table() %>% 
  unnest(cols = c(rmse, cind)) %>% 
...
plot_rmse_vs_cind(res_kNN, x_name = "k_nn")
```

Лучше использовать median (вместо mean), потому что:
```{r}
res_kNN <- sapply(k_to_explore, function(k_nn) {
  pred.kNN <- my_kNN(X_test, X_train, Y_train, my_rmse, k_nn = k_nn, avg_func = mean) 
  list(
    rmse = df_metric(pred.kNN, Y_test, my_rmse),
    cind = df_metric(pred.kNN, Y_test, calc_C_index)
  )
...
k_nn или 5, или (лучше) 53-55.

Поиск наилучшего n_dim:
```{r}
k_to_explore <- 2:ncol(X_train)

res_kNN <- sapply(k_to_explore, function(k_dim) {
  pred.kNN <- my_kNN(X_test, X_train, Y_train, my_rmse, k_dim = k_dim, 
                     k_nn = 55, avg_func = median) 
  list(
    rmse = df_metric(pred.kNN, Y_test, my_rmse),
    cind = df_metric(pred.kNN, Y_test, calc_C_index)
  )
...
plot_rmse_vs_cind(res_kNN, x_name = "k_dim")
```
k_dim = 35 (или 13)

Надо написать автоподбор гиперпараметров:
...
cluster_prediction <- function(X_train_, X_test_, Y_train_, k_clusters = 50,
  ...
  pred_test <- data.table(cl_num = pred_cluster_nums) %>% 
    .[, id := .I] %>%
...
```{r}
cluster_prediction(X_train, X_test, Y_train, k_clusters = 50, avg_func = mean) %>% 
  show_custom_metrics("OneNN RMSE")

cluster_prediction(X_train, X_test, Y_train, k_clusters = 50, avg_func = median) %>% 
  show_custom_metrics("OneNN RMSE")
```

Регулируем 3 параметра: k_clusters, k_dim, avg_func
```{r}
one_dim_exploration <- function(k_clusters_to_explore,
                                X_train_, X_test_, Y_train_, 
                                k_dim = 38,
                                avg_func = mean) {
  res_clust <- sapply(k_clusters_to_explore, function(k_clusters) {
    pred_cl <- cluster_prediction(X_train_, X_test_, Y_train_, k_clusters = k_clusters,
                                  k_dim = k_dim, avg_func = avg_func)
    list(
...
```{r}
one_dim_exploration(k_clusters_to_explore = 2:100, X_train_ = X_train, X_test_ = X_test,
                    Y_train_ = Y_train, k_dim = 38) %>% 
  plot_rmse_vs_cind(x_name = "k_cl", additional_title = paste0("K_dim: ", k_dim))
```


```{r}
for (k_dim_ in c(2, 4, 8, 12, 15, 20, 25, 30, 35, 38, 40, 45, 50)) {
  one_dim_exploration(k_clusters_to_explore = 2:100, k_dim = k_dim_,
                      X_train, X_test, Y_train) %>% 
    plot_rmse_vs_cind(x_name = "k_cl", additional_title = paste0("K_dim: ", k_dim_)) %>% 
    show()
}
```
k_dim = 30, k_cl = 42 ==> C_ind = 10.79, RMSE = 2.38 (seed 42)

k_dim = 20, k_cl = 43 ==> C_ind = 10.16, RMSE = 2.41
k_dim = 25, k_cl = 42 ==> C_ind = 10.2, RMSE = 2.36
k_dim = 25, k_cl = 72 ==> C_ind = 10.42, RMSE = 2.44
k_dim = 35, k_cl = 38 ==> C_ind = 10.28, RMSE = 2.31
