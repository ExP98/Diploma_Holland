...
```{r}
models_by_psytests     <- vector("list", length(psytests))
pred_valid_by_psytests <- vector("list", length(psytests))

for (psytest in psytests) {
  models <- vector("list", 6)
  for (col_i in 1:6) {
    models[[col_i]] <- model_class$new(
      X_train_ = X_train[, str_subset(colnames(X_train), psytest), drop = FALSE], 
      y_train_ = Y_train[, col_i],
      X_test_ = X_test[, str_subset(colnames(X_test), psytest), drop = FALSE], 
      y_test_ = Y_test[, col_i]
    )
  }
...
```{r}
p_idx <- c(rep(TRUE, wide_data[, .N]), rep(FALSE, wide_data2[, .N]))
...
# for (test_lbl in ) {}
X_tr_1test <- X_tr[, str_subset(colnames(X_tr), "BF"), drop = FALSE]
X_ts_1test <- X_ts[, str_subset(colnames(X_ts), "BF"), drop = FALSE]
pred <- perform_stack_MO_regression(my_RandomForest_model, X_tr_1test, Y_tr, X_ts_1test, Y_ts)
```
