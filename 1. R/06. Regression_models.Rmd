...
perform_stack_MO_regression(my_RandomForest_model, X_train, Y_train, X_test, Y_test) %>%
  show_custom_metrics("RF", Y_test)
perform_stack_MO_regression(my_Catboost_model, X_train, Y_train, X_test, Y_test) %>%
  show_custom_metrics("Catboost", Y_test)
```
...
mice_imputation <- function(df_with_na) {
  ...
  df[, (to_imp) := complete(imp, 1)]
  return(df)
}
```

Разделение восстановленных данных:
```{r}
DT <- mice_imputation(na_data)
...
get_L1_L2_glmnet_preds(X_train, Y_train, X_test, Y_test, alpha = 1) # Lasso
```
Работает неплохо, пусть и заметно хуже изначально полных данных. Лучше всего -- GLM Ridge

Разделение данных с пропусками, проверка на XGBoost.
```{r}
set.seed(SEED)
.[ftr_na, trg_na] <- separate_X_y(na_data)
.[X_tr_na, X_ts_na, Y_tr_na, Y_ts_na, ..] <- train_test_split(ftr_na, trg_na, split_idx = split_idx)

perform_stack_MO_regression(my_XGBoost_model, X_tr_na, Y_tr_na, X_ts_na, Y_ts_na) %>% 
  show_custom_metrics(., "XGB with NA", Y_ts_na)

perform_stack_MO_regression(my_XGBoost_model, X_train, Y_train, X_test, Y_test) %>% 
  show_custom_metrics(., "XGB", Y_test)

perform_stack_MO_regression(my_RandomForest_model, X_tr, Y_tr, X_ts, Y_ts) %>% 
  show_custom_metrics("RF with NA", Y_test)
...
```{r}
DT <- mice_imputation(wide_data2)

set.seed(SEED)
.[ftr, trg] <- separate_X_y(DT)
.[X_tr, X_ts, Y_tr, Y_ts, sp_idx] <- train_test_split(ftr, trg)
...
perform_stack_MO_regression(my_Catboost_model, X_tr, Y_tr, X_ts, Y_ts) %>% 
  show_custom_metrics("Catboost", Y_test)
```
...
transform_matrix_completion <- function(fit_obj, DT_new) {
  ...
  M_new <- as.matrix(DT_new[, ..aux_feats])
  M_imp <- softImpute::complete(M_new, fit_obj$fit)
  DT_new <- DT_new[, (aux_feats) := as.data.table(M_imp)]
  return(DT_new)
}
```

Как выбрать rank.max:
```{r}
M_obs <- as.matrix(wide_data2[, ..aux_feats])
sv <- svd(M_obs, nu = 0, nv = 0)$d
plot(sv, type="b", ylab="Singular values", xlab="Component")
```

Восстановим данные там, где есть известные значения:
```{r}
fit_obj <- train_matrix_completion(na_data)
DT <- transform_matrix_completion(fit_obj, na_data)

set.seed(SEED)
.[ftr, trg] <- separate_X_y(new_user_filled)
.[X_tr, X_ts, Y_tr, Y_ts, sp_idx] <- train_test_split(ftr, trg) # , split_idx = split_idx
...
perform_stack_MO_regression(my_Catboost_model, X_tr, Y_tr, X_ts, Y_ts) %>% 
  show_custom_metrics("Catboost", Y_test)
```
Хорошо для GLM, плохо на RF, Catboost.


Данные с реальными пропусками:
```{r}
fit_obj <- train_matrix_completion(wide_data)
DT <- transform_matrix_completion(fit_obj, wide_data2)
...
```{r}
prepare_data <- function(DT, y_cols = NULL) {
  df <- DT %>% as.data.table() %>% copy()
  x_cols <- setdiff(colnames(df), c("id", y_cols))
  
  # создаём маски и заполняем NA нулями
  for (x in x_cols) {
    mcol <- paste0(x, "_mask")
    df <- df %>% 
...
```{r}
set.seed(SEED)
.[ftr, trg] <- separate_X_y(na_data)
DT <- ftr %>% as.data.table() %>% prepare_data(y_cols = paste0("HL_", 1:6))
.[X_tr, X_ts, Y_tr, Y_ts, sp_idx] <- train_test_split(DT, trg, split_idx = split_idx)
...
На wide_data "обучаем", на wide_data2 тестим
```{r}
sp_idx <- c(rep(TRUE, wide_data[, .N]), rep(FALSE, wide_data2[, .N]))

set.seed(SEED)
.[ftr, trg] <- separate_X_y(untd_dt)
.[X_tr, X_ts, Y_tr, Y_ts, ..] <- train_test_split(ftr, trg, split_idx = sp_idx)
X_tr <- prepare_data(X_tr, y_cols = paste0("HL_", 1:6))
X_ts <- prepare_data(X_ts, y_cols = paste0("HL_", 1:6))
...
### 4. Потестовое обучение + обединениее с разными весами

Обучаем на wd, тестим на wd2
```{r}
psytests <- c("BF", "EY", "LN", "SC", "CT") %>% sort()
model_class <- my_RandomForest_model
MO_regr <- stack_MO_regr
weight_model <- particle_swarm_weights
weight_model_params <- list()
```

Потом вставить ..., поменять всё на XXX_
...
for (psytest in psytests) {
  models <- MO_regr(
    ...
    X_train_ = X_train[, str_subset(colnames(X_train), psytest), drop = FALSE], 
    Y_train_ = Y_train,
    X_test_ = X_test[, str_subset(colnames(X_test), psytest), drop = FALSE], 
    Y_test_ = Y_test
  )
  models_by_psytests[[psytest]]     <- models
  pred_valid_by_psytests[[psytest]] <- sapply(models, \(x) x$pred_test)
}
...
predict-стадия
```{r}
# .[ftrs_na, trgs_na] <- separate_X_y(wide_data2)
ftrs_na <- wide_data2[, .SD, .SDcols = !patterns("HL|HL2")]
trgs_na <- wide_data2[, .SD, .SDcols =  patterns("HL|HL2")] %>% as.matrix()

# Нормировать!
.[X_tr_unsc, ..] <- list(features[split_idx, ], features[!split_idx, ])
mean_train <- apply(X_tr_unsc, 2, mean, na.rm = TRUE) %>% c(0, .) # первое -- для id
sd_train <- apply(X_tr_unsc, 2, sd, na.rm = TRUE) %>% c(1, .)

X_na <- scale(ftrs_na, center = mean_train, scale = sd_train) %>% as.data.table()
psytest_preds_by_id <- X_na[, .(id)]

for (psytest in psytests) {
  X_psytest <- X_na[, .SD, .SDcols = patterns(paste0(psytest, "|id"))] %>% drop_na()
  
  pred <- sapply(models_by_psytests[[psytest]], \(mdl) mdl$predict(as.matrix(X_psytest[, -"id"])))
...
#   .[order(id), pred] %>%
#   do.call(rbind, .)
# distr_metric(Y_pred, trgs_na, func = calc_C_index)

# предсказание + настоящая метка + оценка C-index для каждой строки
pred_true_vals <- psytest_preds_by_id %>% copy() %>% 
  .[order(id), .(id, pred)] %>% 
  cbind(trgs_na) %>% 
  .[, .(pred, y_true = list(as.numeric(.SD))), by = id, .SDcols = paste0("HL_", 1:6)] %>% 
  .[, cindex := map2_dbl(pred, y_true, calc_C_index)]

cindex_val <- pred_true_vals$cindex %>% mean()
p <- pred_true_vals$cindex %>% plot_cindex_hist()
```

Обернуть это в функцию
Прогнать на разных моделях и weight_model
Сравнить с другими способами заполнения пропусков

Чтобы прогнать на других моделях, надо везде predict убрать из private
