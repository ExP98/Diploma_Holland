...
```{r}
stack_mtr <- regr_stack_res %>%
  copy() %>% 
  .[!name %in% c("Random Forest (500)", "ExtraTree (500)", "kNN (k = 5)")] %>% 
  extract_matrices()

chain_mtr <- regr_chain_res %>% 
  copy() %>% 
  .[!name %in% c("Random Forest (500)", "ExtraTree (500)", "kNN (k = 5)")] %>% 
  extract_matrices()
...
```{r}
fname <- here("0. Data/1. Output/regression_ensemble_results.xlsx")

write_w_search_to_xlsx(regr_stack_w  , stack_mtr  , "stack", fname)
write_w_search_to_xlsx(regr_stack_w_f, stack_mtr_f, "filtered stack", fname)
write_w_search_to_xlsx(regr_chain_w  , stack_mtr  , "chain", fname)
write_w_search_to_xlsx(regr_chain_w_f, chain_mtr_f, "filtered chain", fname)
```
