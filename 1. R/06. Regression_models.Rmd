...
regularized_lm(X_train, Y_train, X_test, Y_test, alpha = 0) # Ridge
regularized_lm(X_train, Y_train, X_test, Y_test, alpha = 1) # Lasso
...
regularized_lm(X_tr, Y_tr, X_ts, Y_ts, alpha = 0) # Ridge
regularized_lm(X_train, Y_train, X_test, Y_test, alpha = 0) # Ridge

regularized_lm(X_tr, Y_tr, X_ts, Y_ts, alpha = 1) # Lasso
regularized_lm(X_train, Y_train, X_test, Y_test, alpha = 1) # Lasso
```
...
for (i in 1:nrow(w_bytest_comb_config)) {
  ...
  tic()
  .[psytest_models, wght_combos] <- fit_by_tests_ensemble(
    model_class = my_knn_model,
...
### 5. Пример поиска лучшей модели для одного теста
...
for (test_lbl in c("BF", "EY", "CT", "LN", "SC")) {
  ...
  # for (pca_b in c(TRUE, FALSE)) {
    ...
    models_by_tests[[test_lbl]][["Lasso"]] <- regularized_lm(df[["X_train"]], df[["Y_train"]], df[["X_test"]], df[["Y_test"]], alpha = 1)
    models_by_tests[[test_lbl]][["Ridge"]] <- regularized_lm(df[["X_train"]], df[["Y_train"]], df[["X_test"]], df[["Y_test"]], alpha = 0)
