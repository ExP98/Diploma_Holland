...
weighted_cindex(w, mat, label = "equal w")
...
weighted_cindex(w, mat, label = "metric prop")
...
weighted_cindex(w, mat, label = "softmax")
```
...
tic()
appr_w1 <- approx_shapley_cpp(mat, Y_test, matr_cind, R = 500)
toc()
...
```{r}
# 1. Просто без всего
weighted_cindex(filtered_w, filtered_matr, label = "filtered w")


# 2. Шэпли
# shap_fltr_w <- approx_shapley_cpp(filtered_matr, Y_test, matr_cind, R = 1000)
shap_w <- approx_shapley(filtered_matr, Y_test)
weighted_cindex(shap_w, filtered_matr, label = "Approx Shap")


# 3. grid search
gs_w <- grid_search_weights(filtered_matr, Y_test)
weighted_cindex(gs_w, filtered_matr, label = "Grid Search")
...
weighted_cindex(w_qp1, filtered_matr, label = "QP filtered")
weighted_cindex(w_qp2, mat, label = "QP full")


# 5. GA
# быстро
ga_w1 <- genetic_algorithm_weights(filtered_matr, Y_test)
ga_w2 <- genetic_algorithm_weights(mat, Y_test)

weighted_cindex(ga_w1, filtered_matr, label = "GA filtered")
weighted_cindex(ga_w2, mat, label = "GA full")


# 6. pso_optimize_weights
# быстро
pso_w1 <- particle_swarm_weights(filtered_matr, Y_test)
pso_w2 <- particle_swarm_weights(mat, Y_test)

weighted_cindex(pso_w1, filtered_matr, label = "PSO filtered")
weighted_cindex(pso_w2, mat, label = "PSO full")


# 7. bayes_optimize_weights
# долго...
bo_w <- bayes_optimize_weights(filtered_matr, Y_test)
weighted_cindex(bo_w, filtered_matr, label = "Bayes Opt")


# 8. coordinate_optimize_weights
сo_w <- coordinate_optimize_weights(filtered_matr, Y_test)
weighted_cindex(сo_w, filtered_matr, label = "Coordinate optimize")
```
