...
for (feature_num in 1:6) {
  ...
  importance_list[[feature_num]] <- xgb.importance(feature_names = names(X_train), model = xgb_model)
}
...
for (col_i in 2:6) {
  ...
  pred_train_i <- predict(lgb_model, local_train_set) %>% round()
  pred_test_i  <- predict(lgb_model, local_test_set) %>% round()
