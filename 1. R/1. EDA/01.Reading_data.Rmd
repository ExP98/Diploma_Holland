...
get_box_plot <- function(data, rus_rest_name, test_name, x_range = c(0, 27), n_str_wrap = 40) {
  ...
  p <- plot_ly(data = data, x = ~value, y = ~feature, type = "box", 
    ...
    layout(
      title = list(text = paste0("<b>", rus_rest_name, ". Диаграммы размаха факторов</b>")),
...
meta_func <- function(wide_data_, test_name, rus_rest_name, x_range = c(0, 27), n_str_wrap = 40,
                      custom_func = NULL, ...) {
...
```{r message=FALSE, warning=FALSE}
meta_func(wide_data, "cattell_poll", "16-факторный Опросник Кеттелла (16PF)", custom_func = replace_col_using_df, 
          x_range = c(0, 27), var_name = "feature", dict_df = cattell_rus_factors)

meta_func(wide_data, "big_five", "Пятифакторный опросник личности (5PFQ)", x_range = c(0, 80), n_str_wrap = 30)


meta_func(wide_data, "golland", "Тест Голланда (RIASEC)", 
          custom_func = \(dt) mutate(dt, feature = if_else(test_name == "golland", feature %>% golland_enum(), feature)), 
          x_range = c(0, 15))

meta_func(wide_data, "temperament", "Личностный опросник Айзенка (EPQ-R)", x_range = c(-1, 25), n_str_wrap = 30)

meta_func(wide_data, "leonhard_poll", "Опросник Леонгарда-Шмишека", 
          custom_func = \(dt) replace_col_using_df(dt, var_name = "feature", dict_df = leonhard_rus_factors), 
          x_range = c(0, 25))

meta_func(wide_data, "schwartz_poll", "Ценностный опросник Шварца", x_range = c(-1, 25), n_str_wrap = 30)
```

Разделить Шварца на 2 по 10
```{r}
wide_data %>% 
  data_transformation("schwartz_poll")
