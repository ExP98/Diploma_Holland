...
here::here()
# source("../1. R/4. data_preparation.R")
```
...
# data2 <- fread(paste0(here(), "/0. Data/AllTestsData_022025.csv"))
...
data2[, .(N_factors = .N), by = c("id")][, .N, by = "N_factors"]
```
...
```{r}
dt2 <- fread(paste0(here(), "/0. Data/AllTestsData_022025_1.csv"))
  # setnames(old = "ID", new = "id") 

dt2[, .N, by = vk_id][, .N, by = "N"]
...
```{r}
wide_data2[, .N, by = is.na(HL_1) & is.na(HL_2) & is.na(HL_3) & is.na(HL_4) & is.na(HL_5) & is.na(HL_6)]
```
У каждого из 4073 субъектов есть результаты по всем 6 шкалам теста Голланда.
